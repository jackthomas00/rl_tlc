...
LR_ACTOR_OPTIONS = [1e-4, 5e-4, 1e-3]
LR_CRITIC_OPTIONS = [1e-3, 5e-3, 1e-2]
MIN_LR_ACTOR = 1e-5
MIN_LR_CRITIC = 1e-4
REPLAY_BUFFER_SIZE_OPTIONS = [50_000, 100_000, 200_000]
TAU = 0.01            # for soft update of target networks
...
EMERGENCY_BRAKING_PENALTY = 2.0  # Reduced from 5.0 to make penalties less severe
THROUGHPUT_REWARD = 1500.0  # Increased from 2.0 to give more incentive for throughput

BASELINE_WAIT = 500.0  # Increased baseline to handle longer waiting times
...
def compute_reward(junction_id):
    """
    Compute the local reward for an intersection.
    This example uses the negative waiting time and penalties for emergency braking.
    """
    edge_ids = get_junctions_edge_ids(junction_id)
    lane_ids = [lane for edge_id in edge_ids for lane in convert_edge_to_lane_id(edge_id)]
    ...
    for lane_id in lane_ids:
        ...
        waiting_time += min(traci.lane.getWaitingTime(lane_id), MAX_WAITING_TIME)
        
        # Emergency braking penalty
        vehicles = traci.lane.getLastStepVehicleIDs(lane_id)
        for veh_id in vehicles:
            if traci.vehicle.getDecel(veh_id) > 4.5:  # Emergency braking threshold
                emergency_braking_penalty += 1.0
    ...
    # Combine local rewards with weights
    # Scale waiting time penalty to be between -BASELINE_WAIT and BASELINE_WAIT
    waiting_penalty = BASELINE_WAIT - (waiting_time * WAITING_TIME_PENALTY)
    emergency_penalty = -emergency_braking_penalty * EMERGENCY_BRAKING_PENALTY
    smooth_reward = smooth_traffic_reward * 0.5
    
    # Only return local rewards, throughput is handled in global reward
    return waiting_penalty + emergency_penalty + smooth_reward

def compute_global_reward():
    """
    Compute the global reward as the completion rate reward minus the sum of waiting times.
    """
    # Calculate completion rate
    if total_spawned_vehicles > 0:
        completion_rate = total_arrived_vehicles / total_spawned_vehicles
    else:
        completion_rate = 0
    
    # Calculate completion reward
    completion_reward = completion_rate * THROUGHPUT_REWARD
    ...
    # Combine completion reward with waiting time penalty
    return completion_reward - (total_waiting_time * WAITING_TIME_PENALTY)
...
def get_simulation_metrics(actor_loss=None, critic_loss=None, beta_value=None, lr_actor=None, lr_critic=None):
    ...
    # Calculate emergency braking events
    ...
    for veh_id in traci.vehicle.getIDList():
        ...
        if traci.vehicle.getDecel(veh_id) > 4.5:
            emergency_braking_count += 1
    ...
    metrics['emergency_braking_events'] = emergency_braking_count
    metrics['queue_length'] = queue_length
    
    return metrics

# Add early stopping parameters
EARLY_STOPPING_PATIENCE = 20  # Number of episodes without improvement before stopping
MIN_IMPROVEMENT = 0.01  # Minimum improvement required to reset patience
MIN_EPISODES = 50  # Minimum number of episodes before early stopping can occur
MAX_WAITING_TIME = 300  # Maximum allowed average waiting time (seconds)
...
# ------------------
def compute_dynamic_reward(global_state, r_local, r_global, baseline_beta=0.5, lambda_reg=0.01):
    """
    Computes the reward using a dynamically learned β.
    Scales local and global rewards before mixing.
    Also returns a regularization loss for β.
    """
    # Scale the rewards
    r_local_scaled = LOCAL_REWARD_SCALE * r_local
    r_global_scaled = GLOBAL_REWARD_SCALE * r_global
    
    # Convert global_state (raw state) to a torch tensor with a batch dimension.
    global_state_tensor = torch.FloatTensor(global_state).unsqueeze(0)
    
    # Predict β dynamically.
    beta_tensor = beta_network(global_state_tensor)  # Value between 0 and 1.
    beta_value = beta_tensor.item()  # Use this scalar for reward computation.
    
    # Compute the total reward using the scaled rewards.
    reward = beta_value * r_global_scaled + (1 - beta_value) * r_local_scaled
    
    # Regularization: Penalize deviation from a baseline value (e.g., 0.5) to smooth updates.
    beta_reg_loss = lambda_reg * (beta_tensor - baseline_beta) ** 2
    
    return reward, beta_reg_loss
...
# Main Training Loop
# ------------------
def run_training(config, reporter=None):
    # set hyperparameters
    global LR_ACTOR, LR_CRITIC, BATCH_SIZE, REPLAY_BUFFER_SIZE
    ...
    print(f"Trial name: {trial_name}", flush=True)
    
    # Initialize metrics file and get run directory
    metrics_file, run_dir, config_file = init_metrics_file(trial_name)
    
    global total_arrived_vehicles, total_spawned_vehicles
    total_arrived_vehicles = 0
    total_spawned_vehicles = 0
    
    # Initialize state normalizer with raw global state dimension.
    global state_normalizer, actors, critics, target_actors, target_critics, actor_optimizers, critic_optimizers, actor_schedulers, critic_schedulers, beta_network, beta_optimizer, replay_buffer
    ...
    beta_optimizer = optim.Adam(beta_network.parameters(), lr=1e-3)
    
    # Handle checkpoint loading from config instead of command line
    start_episode = 0
    if "load_checkpoint" in config and config["load_checkpoint"] is not None:
        load_model_checkpoint(config["load_checkpoint"])
        print(f"Loaded checkpoint from episode {config['load_checkpoint']}")
        # Reduce learning rates for fine-tuning.
        for optimizer in actor_optimizers:
            for param_group in optimizer.param_groups:
                param_group['lr'] = LR_ACTOR * 0.1
        for optimizer in critic_optimizers:
            for param_group in optimizer.param_groups:
                param_group['lr'] = LR_CRITIC * 0.1
        start_episode = config["load_checkpoint"]
    ...
    for episode in range(start_episode, NUM_EPISODES):
        ...
        for t in range(TIME_STEPS):
            ...
            for i in range(NUM_AGENTS):
                ...
                r, beta_reg_loss = compute_dynamic_reward(global_state, r_local, r_global_step)
                total_beta_reg_loss += beta_reg_loss
                local_rewards.append(r)
                episode_reward += r
                # Track per-agent rewards
                per_agent_rewards[i].append(r)
            
            # Apply regularization loss to beta network.
            beta_optimizer.zero_grad()
            total_beta_reg_loss.backward()
            beta_optimizer.step()
        ...
        # Compute average waiting time per junction and throughput from SUMO.
        total_waiting_time = get_total_waiting_time()
        avg_waiting_time = total_waiting_time / NUM_AGENTS
        ...
        # Log metrics
        log_metrics(
            metrics_file, episode+1, episode_reward, avg_waiting_time, 
        ...
        if (episode + 1) % 50 == 0:
            save_model_checkpoint(episode + 1, run_dir, epsilon)
        ...
        if episode >= MIN_EPISODES:
            ...
            # Check if waiting time is too high
            if avg_waiting_time > MAX_WAITING_TIME:
                print(f"Stopping due to high waiting time: {avg_waiting_time}")
                early_stop = True
                
            # Check if we've run out of patience
            if patience_counter >= EARLY_STOPPING_PATIENCE:
                print(f"Stopping due to no improvement for {EARLY_STOPPING_PATIENCE} episodes")
                early_stop = True
        
        # Compute reward variance at the end of each episode
        reward_variance = compute_reward_variance(per_agent_rewards)
        
        # Report to Ray Tune with additional metrics
        tune.report(
            metrics={
                "episode_reward": episode_reward,
                "avg_waiting_time": avg_waiting_time,
                "throughput": sim_metrics['vehicles_in_network'],
                "avg_speed": sim_metrics['average_speed'],
                "emergency_braking": sim_metrics['emergency_braking_events'],
                "queue_length": sim_metrics['queue_length'],
                "actor_loss": actor_loss,
                "critic_loss": critic_loss,
                "beta_value": beta_network(torch.FloatTensor(global_state)).mean().item(),
                "reward_variance": reward_variance  # Add reward variance metric
            }
        )
    ...
    # Save final models.
    save_model_checkpoint("final", run_dir)
...
# ------------------
def init_metrics_file(trial_name, trial_dir=None):
    """
    ...
    if trial_dir is None:
        # For local testing without Ray
        run_dir = os.path.join("/app/src/runs/", trial_name)
    else:
        # Use Ray Tune's trial directory
        run_dir = trial_dir
    ...
    config = {
        "lr_actor": LR_ACTOR,
        "lr_critic": LR_CRITIC,
        "min_lr_actor": MIN_LR_ACTOR,
        "min_lr_critic": MIN_LR_CRITIC,
        "gamma": GAMMA,
        ...
        "emergency_braking_penalty": EMERGENCY_BRAKING_PENALTY,
    ...
    return metrics_file, run_dir, config_file

def log_metrics(metrics_file, episode, episode_reward, avg_waiting_time, 
    ...
    with open(metrics_file, "a", newline="") as csvfile:
        ...
        writer.writerow([
            ...
            episode_reward, 
            avg_waiting_time, 
            sim_metrics['vehicles_in_network'],
            sim_metrics['arrived_vehicles'],
            sim_metrics['average_speed'],
            sim_metrics['average_travel_time'],
            sim_metrics['emergency_braking_events'],
...
def save_model_checkpoint(episode, run_dir, epsilon=None):
    """
    Save the current model checkpoint in the run-specific directory.
    """
    checkpoint_dir = os.path.join(run_dir, f"checkpoint_{episode}")
    os.makedirs(checkpoint_dir, exist_ok=True)
    
    # Save models
    for i in range(NUM_AGENTS):
        torch.save(actors[i].state_dict(), os.path.join(checkpoint_dir, f"actor_agent_{i}.pth"))
        torch.save(critics[i].state_dict(), os.path.join(checkpoint_dir, f"critic_agent_{i}.pth"))
    
    # Save replay buffer and state normalizer
    replay_buffer.save(os.path.join(checkpoint_dir, "replay_buffer.pkl"))
    state_normalizer.save(os.path.join(checkpoint_dir, "state_normalizer.pkl"))
    
    # Save current epsilon value
    with open(os.path.join(checkpoint_dir, "epsilon.json"), "w") as f:
        json.dump({"epsilon": epsilon}, f)
    
    print(f"Checkpoint saved for episode {episode} in {checkpoint_dir}")

def load_model_checkpoint(episode, run_dir):
    """
    Load the model checkpoint from the specified run directory.
    """
    checkpoint_dir = os.path.join(run_dir, f"checkpoint_{episode}")
    
    for i in range(NUM_AGENTS):
        actors[i].load_state_dict(torch.load(os.path.join(checkpoint_dir, f"actor_agent_{i}.pth")))
        critics[i].load_state_dict(torch.load(os.path.join(checkpoint_dir, f"critic_agent_{i}.pth")))
    
    replay_buffer.load(os.path.join(checkpoint_dir, "replay_buffer.pkl"))
    state_normalizer.load(os.path.join(checkpoint_dir, "state_normalizer.pkl"))
    
    # Load epsilon value
    with open(os.path.join(checkpoint_dir, "epsilon.json"), "r") as f:
        global epsilon
        epsilon = json.load(f)["epsilon"]
    
    print(f"Checkpoint loaded from episode {episode} in {checkpoint_dir}")

def log_learning_rates(episode, run_dir):
    ...
    # Log to file in the run directory
    with open(os.path.join(run_dir, 'learning_rates.log'), 'a') as f:
...
def get_total_waiting_time():
    """
    Calculate the total waiting time across all lanes in the network.
    """
    total_waiting_time = 0.0
    for i in range(NUM_AGENTS):
        edge_ids = get_junctions_edge_ids(junctions[i])
...
if __name__ == '__main__':
    # Initialize Ray with custom object store directory
    ray.init(
        _temp_dir="/mnt/docker/sumo_tmp/ray_temp",
        object_store_memory=8 * 1024 * 1024 * 1024  # 8GB (leaving some headroom)
    )
    ...
    search_space = {
        ...
        "replay_buffer_size": tune.choice([25000, 50000, 100000, 200000]),
    ...
    try:
        analysis = tune.run(
            run_training,
            config=search_space,
            storage_path="/app/src/ray_results",
            stop={"training_iteration": 50},
            resources_per_trial={"cpu": 1, "gpu": 0},
            num_samples=60,  # Number of times to sample from the hyperparameter space
            name="traffic_control",  # Name of the experiment
            scheduler=tune.schedulers.ASHAScheduler(
                ...
                stop_last_trials=True  # Ensure we get results from the best trials
            ),
            search_alg=OptunaSearch(
                metric="episode_reward",
                mode="max"
            ),
            keep_checkpoints_num=3,  # Keep checkpoints for the best trials
            resume="AUTO"  # Automatically resume from checkpoints if available
        )
        
        # Get the best trial's results
        best_trial = analysis.get_best_trial("episode_reward", mode="max")
        best_config = analysis.get_best_config("episode_reward", mode="max")
        best_result = analysis.get_best_result("episode_reward", mode="max")
        
        print("\nBest trial results:")
        print(f"Trial ID: {best_trial.trial_id}")
        print(f"Best config: {best_config}")
        print(f"Best episode reward: {best_result['episode_reward']}")
        print(f"Average waiting time: {best_result['avg_waiting_time']}")
        print(f"Throughput: {best_result['throughput']}")
